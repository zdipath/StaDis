...
class CLAM_dropout(nn.Module):
    ...
    def forward_threshold(self, h, threshold=1e6):
        ...
        A = F.softmax(A, dim=1)  # softmax over N 
        M = torch.mm(A, h) 
        M = M.clip(max=threshold)
        logits = self.classifiers(M)
        return logits
    

    def probability_patch(self, h):
        _, h = self.attention_net(h)
        logits = self.classifiers(h)
        return logits
    

class CLAM_ensemble(nn.Module):
    def __init__(self, gate = True, size_arg = "small", dropout = 0., k_sample=8, n_classes=2,
        instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False, embed_dim=1024):
        super().__init__()
        self.size_dict = {"small": [embed_dim, 512, 256], "big": [embed_dim, 512, 384]}
        ...
        self.classifiers5 = nn.Linear(size[1], n_classes)
        instance_classifiers = [nn.Linear(size[1], 2) for i in range(n_classes)]
        self.instance_classifiers = nn.ModuleList(instance_classifiers)
        self.k_sample = k_sample
        self.instance_loss_fn = instance_loss_fn
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        M = torch.mm(A, h) 
        logits = self.classifiers(M)
        logits2 = self.classifiers2(M)
        logits3 = self.classifiers3(M)
        logits4 = self.classifiers4(M)
        logits5 = self.classifiers5(M)
        Y_hat = torch.topk(logits, 1, dim = 1)[1]
        Y_prob = F.softmax(logits, dim = 1)
        if instance_eval:
            results_dict = {'instance_loss': total_inst_loss, 'inst_labels': np.array(all_targets), 
